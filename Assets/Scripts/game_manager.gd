...
func next_level():
	if float(PlayerData.times[completed_area - 1]) > float(total_time):
		PlayerData.times[completed_area - 1] = float(total_time)
	var completed_area = current_area
	Dialogue.kid = false
	pause = false
	...
	if not current_area == 30:
		...
		if has_star == 1:
			...
			has_star = 0
		if PlayerData.times[completed_area - 1] > total_time:
			PlayerData.times[completed_area - 1] = total_time
